(() => {
    ...
    function initSliders() {
        if (document.querySelector(".slider-creators")) new core(".slider-creators", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 5,
        ...
        if (document.querySelector(".slider-quick-bytes")) new core(".slider-quick-bytes", {
            modules: [ Navigation, Pagination ],
            observer: true,
            observeParents: true,
            speed: 800,
            pagination: {
                el: ".slider-quick-bytes__bullets",
                clickable: true
            },
            navigation: {
                prevEl: ".quick-bytes-prev",
                nextEl: ".quick-bytes-next"
            },
            on: {}
        });
    }
    ...
    function applyFocusVisiblePolyfill(scope) {
        ...
        function focusTriggersKeyboardModality(el) {
            ...
            if ("INPUT" === tagName && inputTypesAllowlist[type] && !el.readOnly) return false;
            if ("TEXTAREA" === tagName && !el.readOnly) return false;
    ...
    if ("undefined" !== typeof window && "undefined" !== typeof document) {
        ...
        var focus_visible_event;
        try {
            focus_visible_event = new CustomEvent("focus-visible-polyfill-ready");
        } catch (error) {
            focus_visible_event = document.createEvent("CustomEvent");
            focus_visible_event.initCustomEvent("focus-visible-polyfill-ready", false, false, {});
        }
        window.dispatchEvent(focus_visible_event);
    }
 ...
 */
    ...
    window.addEventListener("load", (function(event) {
        if (document.querySelector("[data-filter]")) {
            let filterCats = document.querySelectorAll("[data-filter-cat]");
            for (let cat of filterCats) {
                const filterCatValue = cat.dataset.filterCat;
                cat.addEventListener("click", (function() {
                    if (document.querySelector("[data-showmore]._showmore-active")) {
                        document.querySelector("[data-showmore]").classList.remove("_showmore-active");
                        let showMoreContent = document.querySelector("[data-showmore-content]");
                        const showmoreContent = document.querySelector("[data-showmore-content]").dataset.showmoreContent;
                        _slideUp(showMoreContent, 500, showmoreContent);
                    }
                    if (document.querySelector("[data-filter-item]")) {
                        let filterItems = document.querySelectorAll("[data-filter-item]");
                        for (let item of filterItems) {
                            const filterItemValue = item.dataset.filterItem;
                            if ("all" !== filterCatValue) if (filterCatValue !== filterItemValue) item.classList.add("_hidden"); else item.classList.remove("_hidden"); else item.classList.remove("_hidden");
                        }
                    }
                    breakpointChecker();
                }));
        ...
        let masonryOptions = {
            ...
            columnWidth: ".fillter-grid-sizer",
            percentPosition: true,
            gutter: 15
        };
        let msnry, msnry2;
        function masonryInit() {
            if (document.querySelector(".fillter-grid")) msnry = new Masonry(".fillter-grid", masonryOptions);
            if (document.querySelector(".fillter-grid-2")) msnry2 = new Masonry(".fillter-grid-2", masonryOptions);
        }
        const breakpoint = window.matchMedia("(min-width: 500px)");
        const breakpointChecker = function() {
            if (true === breakpoint.matches) setTimeout((() => {
                masonryInit();
            }), 300); else if (false === breakpoint.matches) {
                if (void 0 !== msnry) msnry.destroy();
                if (void 0 !== msnry2) msnry2.destroy();
            }
        };
        breakpoint.addListener(breakpointChecker);
        breakpointChecker();
    }));
    const weather = document.querySelector(".widget-weather");
    async function loadWeather(e) {
        weather.innerHTML = `\n      <div class="widget-weather__loader-wrapper loader-wrapper loader">\n         <div class="loader__item"></div> \n      </div>`;
        const server = "https://api.openweathermap.org/data/2.5/weather?units=metric&q=Kyiv&lang=ua,uk&appid=08c95fdcbae5170fe1d30bcb3decf962";
    ...
    function weathertTanslation(data) {
        const weather_Ru = {
        ...
        for (let key in weather_Ru) {
            weather_Ru[key];
            if (data === key) return weather_Ru[key];
        }
    }
    function getWeather(data) {
        const location = weathertTanslation(data.name) ? weathertTanslation(data.name) : data.name;
        const weatherTemp = Math.round(data.main.temp);
        const weatherStatus = weathertTanslation(data.weather[0].main) ? weathertTanslation(data.weather[0].main) : data.weather[0].main;
        const weatherIcon = data.weather[0].icon;
        console.log(data);
        const template = `\n   <div class="widget__title widget-weather__title weather-location title-icon-right">${location}</div>\n   <div class="widget-weather__body body-wather">\n      <div class="body-wather__label">${weatherStatus}</div>\n      <div class="body-wather__temperature-value"><span class="weather-temp">${weatherTemp}</span><sup>o</sup><span class="wather-degree">c</span></div>\n      <div class="body-wather__icon">\n         <img src="http://openweathermap.org/img/wn/${weatherIcon}.png" alt="weatherIcon"/>\n      </div>\n   </div>\n   <div class="widget-weather__footer footer-weather">\n      <button class="footer-weather__button weather-celsius" type="button">Цельсий</button>\n      <button class="footer-weather__button footer-weather__button_active weather-fahrenheit" type="button"> Фаренгейт</button>\n   </div>\n   `;
        weather.innerHTML = template;
        const celsius = document.querySelector(".weather-celsius");
        const fahrenheit = document.querySelector(".weather-fahrenheit");
        const tempItem = document.querySelector(".weather-temp");
        const degreeItem = document.querySelector(".wather-degree");
        if (celsius) celsius.addEventListener("click", (function() {
            weatherCelsius(weatherTemp);
        }));
        if (fahrenheit) fahrenheit.addEventListener("click", (function() {
            weatherFahrenheit(weatherTemp);
        }));
        function weatherCelsius(data) {
            tempItem.innerText = weatherTemp;
            degreeItem.innerText = "c";
        }
        function weatherFahrenheit(data) {
            tempItem.innerText = Math.round(9 * weatherTemp / 5 + 32);
            degreeItem.innerText = "f";
        }
    }
